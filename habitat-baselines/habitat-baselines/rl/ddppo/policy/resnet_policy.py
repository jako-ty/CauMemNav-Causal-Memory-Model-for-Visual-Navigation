...
class PointNavResNetNet(Net):
    ...
    def __init__(
        ...
        if force_blind_policy:
            ...
            rgb_keys = ["rgb_front", "rgb_right", "rgb_back", "rgb_left"]
            has_multi_rgb = all(k in observation_space.spaces for k in rgb_keys)

            if has_multi_rgb:

                orig_shape = observation_space.spaces["rgb_front"].shape
                pano_shape = (orig_shape[0], orig_shape[1] * 4, 3)
                use_obs_space = spaces.Dict({"rgb": spaces.Box(low=0, high=255,shape=pano_shape,dtype=np.uint8)})
                self._is_pano_from_4views = True
            else:
                use_obs_space = spaces.Dict(
                    {
                ...
                self._is_pano_from_4views = False
        ...
        if backbone.startswith("resnet50_clip"):
            ...
            self.visual_encoder = ResNetEncoder(
                use_obs_space,
                baseplanes=resnet_baseplanes,#主干网络的初始通道数
                ngroups=resnet_baseplanes // 2,#分组归一化
                make_backbone=getattr(resnet, backbone),#动态获取 ResNet 构造函数
                normalize_visual_inputs=normalize_visual_inputs,#是否对输入图像做归一化
            )
    ...
    def forward(
        ...
        if getattr(self, "_is_pano_from_4views", False):
            views = [
            ...
            panorama = torch.cat(views, dim=2)
            observations["rgb"] = panorama


        x = []
        aux_loss_state = {} #存储中间表示
        ...
        if not self.is_blind:
            ...
            similarity = torch.sum(obs_norm * goal_norm, dim=1)  # [B], 范围 [-1, 1]
        ...
        gps = observations[EpisodicGPSSensor.cls_uuid].float()  # [B, 2]
        compass = observations[EpisodicCompassSensor.cls_uuid].float()  # [B, 1]

        agent_poses = torch.cat([gps, compass], dim=-1)  # [B, 3]
