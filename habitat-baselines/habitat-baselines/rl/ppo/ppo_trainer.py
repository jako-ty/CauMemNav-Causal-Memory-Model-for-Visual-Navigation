...
class PPOTrainer(BaseRLTrainer):
    ...
    def _create_agent(self, resume_state, **kwargs) -> AgentAccessMgr:
        ...
        #从注册表中根据配置的 agent.type 获取对应的 AgentAccessMgr 类
    ...
    def _init_envs(self, config=None, is_eval: bool = False):
        ...
        #创建 EnvironmentSpec 对象，记录第一个环境的观测空间和动作空间。
        ...
        #收集那些只应在 rank 0 进程（主进程）上记录的度量名称，避免在所有进程上重复计算和记录。
    ...
    def _init_train(self, resume_state=None):
        #尝试从配置指定的检查点目录加载之前训练的状态
        if resume_state is None:
            resume_state = load_resume_state(self.config)

        if resume_state is not None:
            #配置中 load_resume_state_config=False，则抛出错误
        ...
        #调用 _init_envs 创建环境，并通过 get_device 获取计算设备
        self._init_envs()
    ...
    def _compute_actions_and_step_envs(self, buffer_index: int = 0):
        ...
        #获取当前步的数据（观测、隐藏状态、掩码等），并调用策略的 act 方法采样动作。
        ...
        #将采样的动作（如果是连续动作则先 clip 到合法范围）异步地发送给对应的环境进程执行。
        ...
        #将采样得到的动作、值函数预测等数据插入到 rollout 存储器中
        with g_timer.avg_time("trainer.obs_insert"):
    ...
    def _collect_environment_result(self, buffer_index: int = 0):
        ...
        with g_timer.avg_time("trainer.update_stats"):
            ...
            #episode 结束，将 current_episode_reward 重置为 0。
            self.current_episode_reward[env_slice].masked_fill_(
                done_masks, 0.0
            )

        #如果使用静态编码器，计算并缓存新的视觉特征。
        if self._is_static_encoder:
            ...
            #将新的观测、奖励和掩码插入 rollout 存储器，并将 rollout 指针向前推进一步。
        self._agent.rollouts.insert(
    ...
    #获取 rollout 的最后一步数据，并用策略的价值网络计算下一个状态的价值
    def _update_agent(self):
        ...
        #使用 next_value 和 GAE（广义优势估计）参数计算整个 rollout 的回报（returns）。
        ...
        #将智能体设置为训练模式，调用 updater来执行算法的更新步骤
        self._agent.train()

        losses = self._agent.updater.update(self._agent.rollouts)

        #重置 rollout 存储器和智能体的内部状态。
    ...
    def _coalesce_post_step(
        ...
        #将所有进程的统计量相加，然后将聚合后的结果追加到 window_episode_stats 的 deque 中。
    ...
    def _training_log(
        ...
        #使用 TensorBoard writer 记录平均奖励、平均度量和损失。
        writer.add_scalar(
            "reward",
    ...
    def train(self) -> None:
        ...
        with (
            ...
            while not self.is_done():
                ...
                #检查全局的 EXIT 信号，如果被设置，则保存状态并重新排队作业（requeue_job），然后退出。
                if EXIT.is_set():
                ...
                #在分布式模式下，通知其他进程自己已经完成了一次 rollout。
                if self._is_distributed:
                    self.num_rollouts_done_store.add("num_done", 1)

                #更新智能体，增加更新计数，聚合统计和损失，记录日志。
    ...
    def _eval_checkpoint(
        ...
        #创建 Evaluator 实例并调用其 evaluate_agent 方法执行评估。
