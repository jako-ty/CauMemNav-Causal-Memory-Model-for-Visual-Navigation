...
def build_pack_info_from_episode_ids(
    ...
    #填充 num_seqs_at_step 和 select_inds
    offset = 0
    prev_len = 0
...
#从 dones（T×N 布尔数组）自动生成 episode 信息。
...
#将rnn输出还原为原始序列，并提取每个环境最后一个episode 的 hidden state。
